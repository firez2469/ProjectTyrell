...
public class MapMeshGeneration : MonoBehaviour
{
    ...
    private void CreateMeshTiles(MapData.PolygonData polygonData, List<MapData.TileData> tileDataList)
    {
        ...
        for (int i = 0; i < polygonData.polygons.Count; i++)
        {
            ...
            if (tileDataMap.TryGetValue(tileName, out MapData.TileData tileData))
            {
                ...
                tile.type = tileData.type.ToLower() == "ocean" ? MeshTileType.Sea : MeshTileType.Land;
                tile.biome = tileData.type;
    ...
    private void CreateTileGameObjects()
    {
        ...
        foreach (var tile in tiles.Values)
        {
            ...
            if (tile.type == MeshTileType.Sea)
            {
                meshRenderer.sharedMaterial = seaMaterial;
            }
            else
            {
                if (!biomeMaterialMap.TryGetValue(tile.biome, out Material biomeMat))
                {
                    biomeMat = landMaterial;
                }
                meshRenderer.sharedMaterial = biomeMat;
            }
    ...
    private Dictionary<string, Material> biomeMaterialMap = new Dictionary<string, Material>();

    private void GenerateBiomeMaterials()
    {
#if UNITY_EDITOR
        string materialFolder = "Assets/Resources/PolyGen/BiomeMaterials";
        ...
        biomeMaterialMap.Clear();

        foreach (var kvp in biomeColorMap)
        {
            string biome = kvp.Key;
            Color color = kvp.Value;

            // Create new material from landMaterial
            Material biomeMat = new Material(landMaterial);
            biomeMat.color = color;
            biomeMat.name = $"BiomeMaterial_{biome}";

            biomeMaterialMap[biome] = biomeMat;

#if UNITY_EDITOR
            string sanitizedBiomeName = biome.Replace(" ", "_").Replace("/", "_");
            string assetPath = $"{materialFolder}/{sanitizedBiomeName}.mat";

            // Only create if not already present or regenerateMeshes is true
            if (!File.Exists(assetPath) || regenerateMeshes)
            {
                UnityEditor.AssetDatabase.CreateAsset(biomeMat, assetPath);
            }
#endif
        }
